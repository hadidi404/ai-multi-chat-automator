...
//
// WHY OAUTH AND NOT JUST A FOLDER LINK:
//   A Drive folder URL is an address, not a key. Writing into someone's Drive
//   always needs an authorised token, so the user connects their Google account
//   once and we keep the refresh token locally.
//
// SCOPE:
//   drive.file — per-file access. We can create files (including inside a
//   folder the user names) and manage the ones we created. We cannot read
//   anything else in their Drive, which is the least privilege that works.
//
// Everything is stored in drive-config.json, which is gitignored: it holds the
// OAuth client secret and the refresh token.
// ─────────────────────────────────────────────────────────────────────────────

// Two files on purpose:
//   drive-client.json — the OAuth app itself (Client ID + Secret). You create
//     this ONCE and ship it in the folder you hand to teammates, so they never
//     see a Cloud Console. For a desktop OAuth client the secret is not really
//     secret; Google expects it to be distributed inside the app.
//   drive-config.json — per person: which folder, and their own refresh token.
const CLIENT_FILE = path.join(__dirname, '..', 'drive-client.json');
 ...
 */
function readClient() {
  const shipped = readJson(CLIENT_FILE);

  if (shipped.clientId && shipped.clientSecret) {
    return { clientId: shipped.clientId, clientSecret: shipped.clientSecret, preconfigured: true };
  }
