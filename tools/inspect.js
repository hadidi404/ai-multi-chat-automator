...
// Selector inspector
//
// Sends ONE question to ONE bot and then just watches, recording every half
// second what the real reader would have seen: which response selectors match,
// how many blocks each one has, how much text is in them, and whether the stop
// button is still visible.
//
// This exists because the two failures that look identical from the sheet —
// "the answer was never found" and "the answer was found but read too early" —
// can only be told apart by watching the page over time. The run itself cannot
// print this without spamming a normal run's log.
//
// USAGE:
//   node tools/inspect.js perplexity "best real estate agents in Anaheim"
//
// Writes debug/<bot>-<timestamp>.log and a .png screenshot next to it.
// ─────────────────────────────────────────────────────────────────────────────
